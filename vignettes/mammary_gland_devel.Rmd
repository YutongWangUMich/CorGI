...
```{r}
pal <- SingleCellExperiment(assay = list(counts = as.matrix(pal_raw[,3:ncol(pal)])))
dim(pal)
...
head(colnames(giraddi))
head(giraddi$gene_id)
...
giraddi <- subsample_cells(giraddi,1000)
...
lapply(X = seq_along(gene_sets),
       FUN = function(i){
         ...
         emb <- embeddings[[i]]
         plot_dimensionality_reduction(emb,batch,cell_type,gs_name,"MDS") +
           scale_shape_manual(values = c(1, 16)) +
           scale_color_manual(values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7"))
       }) -> plts
plts
