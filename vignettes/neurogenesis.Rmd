...
```{r}
gene_sets[[4]] <- union(top_n_genes(gene_loadings[,1],114), top_consensus)
gene_sets[[5]] <- union(top_n_genes(gene_loadings[,2],109), top_consensus)
gene_sets[[6]] <- union(top_n_genes(gene_loadings[,3],105), top_consensus)

names(gene_sets)[c(4,5,6)] <- paste0("CC",1:3)

gene_sets[[3]] <- union(gene_sets[[1]], gene_sets[[6]])
names(gene_sets)[3] <- "CorGI+CCA3"
lapply(gene_sets, length)
```
