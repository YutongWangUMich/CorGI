...
```{r}
shin <- readRDS(file = "/Users/girasole/github_repos/scRNAseq_datasets/aux_files/2019-03-03-shin-cell-stem-cell-2015-adult-neurogenesis/shin.Rds")
shin$cell_type1 <- "Shin"
```
...
combined <-
  combine_sces(
    sce_list = list(Dulken = dulken, Llorens = llorens),
  )
...
```{r}
table(combined$cell_type, combined$batch)
```
...
qplot(mds[,1],mds[,2],color = combined$cell_type,shape = combined$batch) + ggtitle("Spearman rho MDS using all the genes")
```
...
fs <- list(
  ...
  kmeans_obj = function(X, Y){
    emb <- fs_var_o[["emb"]]
    kmeans_output <- cluster::clara(emb, k = 3) 
    kmeans_output$objective/mean(kmeans_output$diss)
  },
...
saveRDS(corgi_out, file = "~/corgi_output_neurogenesis_semisupervised.rds")

rm(corgi_output_neurogenesis)
corgi_output_neurogenesis <- corgi_out
...
head(corgi_output_neurogenesis)
```

```{r}
corgi_output_neurogenesis <- readRDS(file = "~/corgi_output_neurogenesis_semisupervised.rds")
# corgi_output_neurogenesis
corgi_scores <- corgi_output_neurogenesis[,1:3]/corgi_output_neurogenesis[,4]
plot(corgi_scores[,"principal_curve"], corgi_scores[,"kmmd_statistic"])

hist(corgi_scores[,"kmmd_statistic"])
```
...
kmmd_statistic = function(X, Y){
    rk <- function(x) apply(x, 2, rank)
    return(kernlab::kmmd(t(rk(X)), t(rk(Y))))
    # kmmd_out@mmdstats[2]
}
summary(corgi_scores[,"kmmd_statistic"])
stats::mad(corgi_scores[,"kmmd_statistic"])
sd(corgi_scores[,"kmmd_statistic"])
...
n_genes <- length(union(corgi_gene_set,consensus34))
...
gene_sets[["Cons134"]] <- names(top_consensus_genes)[1:n_genes]
...
library(Seurat)
CCA_gene_loading <- Seurat_CCA_GeneLoading(counts(dulken),counts(llorens),k = 3)
# CCA_gene_loading <-CCSpearman_GeneLoading(normcounts(yan),counts(deng),k = 4)[,2:4]
# for(i in 1:3){
#   f <- function(n) union(top_n_genes(CCA_gene_loading[ , i]^2, n), consensus34)
#   gene_sets[[paste0("CCA",i)]] <- 
#     get_gene_set_with_markers(f, n_genes)
# }

leverage_scores <- rowSums(CCA_gene_loading^2)
f <- function(n) union(top_n_genes(leverage_scores, n), consensus34)
gene_sets[["CCA-GL"]] <- 
  get_gene_set_with_markers(f, n_genes)
...
```{r, eval = F}
library(tidyr)
b <- intersect(rownames(corgi_scores), rownames(CCA_gene_loading))
plot_two_dfs(data.frame(CCA_gene_loading[b, ]^2, 
                        lev_score = rowSums(CCA_gene_loading[b, ]^2)),
             data.frame(corgi_scores[b, ]))
```
...
ggsave(filename = "neurogenesis_MDS.eps",device = "eps",plot = neurogenesis_MDS,path = "~/CorGI_figures/neurogenesis",width = 8,height = 8)
cohen_kappa_plot
```
...
row_ann <- data.frame(leverage_score = leverage_scores[rownames(R)],
                      is_consensus = rownames(R) %in% consensus34)

cell_type_colors <- my_color_palette[1:4]
...
scmap_output$table
scmap_output$overall
```

Using the C134 gene set
```{r}
gene_set <- gene_sets[["Leverage"]]
corgi::run_scmapCluster(
  query = llorens,
  ref = dulken[,dulken$cell_type1!="Ast"],
  gene_set = gene_set,
...
cluster_coherence_plot <- plot_mapping_accuracy_comparison(comparison_results) +
...
```{r}
get_mnn_embeddings <- function(gene_sets){
  lapply(
    X = gene_sets,
    FUN = function(gene_set) {
...
```{r}
emb_name <- "MNNPC"
get_top_panel2 <- function(embeddings){
get_scatterplots(embeddings,
                 batch,
                 cell_type) -> dr_plots
...
get_top_panel2(get_mnn_embeddings(gene_sets[grp1]))
get_top_panel2(get_mnn_embeddings(gene_sets[grp2]))
```
...
# length(scmap_output$scmap_cluster_labs) == ncol(llorens)
kable(scmap_output$table)

kable(scmap_output$overall)

# scmap_output$scmap_cluster_labs[,"Reference"]
```
