...
lapply(X = seq_along(gene_sets),
       FUN = function(i){
         ...
         emb <- embeddings[[i]]
         plot_dimensionality_reduction(emb,batch,cell_type2,gs_name,"MDS") +
           scale_shape_manual(values = c(1, 16)) +
...
knn.predict <- function(test.suite,cell.meta.data.pred = "cell_type1", test.batches, train.batches, gene.sets.use =NULL, methods.use = NULL, params=5*(1:6)){
  knn.outputs <- list()
  dr.ids <- get.main.loop(test.suite,gene.sets.use,methods.use)
  knn.params <- data.frame(dr.id = rep(dr.ids,each = length(params)),
                           param = rep(params,length(dr.ids)))
  j <- 1
  for(i in dr.ids){
    for(param in params){
      plt.dat <- prepare.data.for.plotting(test.suite,i)
      df <- plt.dat$df
      # df <- prepare.data.for.plotting(test.suite,i)$df
      truth <- df[,cell.meta.data.pred]
      test <- df$batch %in% test.batches
      train <- df$batch %in% train.batches
      class::knn(train = df[train,1:2],
                 test = df[test,1:2],
                 cl= truth[train],
                 k = param) -> predictions
      knn.outputs[[j]] <- data.frame(truth = truth[test], pred = predictions)
      j <- j + 1
    }
  }
  test.suite$knn.predict.outputs <- knn.outputs
  test.suite$knn.predict.params <- knn.params
  test.suite$knn.predict.benchmarks <- lapply(X = knn.outputs,FUN = cont.tab.to.TPR.FPR)
  test.suite$knn.predict.meta.data <- 
    list(test.batches=test.batches,train.batches=train.batches,cell.meta.data.pred=cell.meta.data.pred)
  return(test.suite)
}
