...
```{r}
gene_loadings <- Seurat_CCA_GeneLoading(logcounts(IN), logcounts(LB), k = 3)
...
corgi_scores <- corgi_output_liver_bud[,1]/corgi_output_liver_bud[,2]
names(corgi_scores) <- rownames(corgi_output_liver_bud)
```

## Selecting the top CorGI genes

```{r}
corgi_gene_set <- top_n_genes(-corgi_scores, 100)
```
...
```{r}
library(Seurat)
CCA_gene_loading <- Seurat_CCA_GeneLoading(logcounts(IN),logcounts(LB),k = 3)
# CCA_gene_loading <-CCSpearman_GeneLoading(normcounts(yan),counts(deng),k = 4)[,2:4]
for(i in 1:3){
  gene_sets[[paste0("CCA",i)]] <- top_n_genes(CCA_gene_loading[ , i]^2, n_genes)
}

leverage_scores <- rowSums(CCA_gene_loading^2)
gene_sets[["Leverage"]] <- top_n_genes(leverage_scores, n_genes)

lapply(gene_sets, length)
```

```{r}
b <- intersect(rownames(data.frame(corgi_scores)), rownames(gene_loadings))

plot_two_dfs(data.frame(gene_loadings[b, ]^2, 
                        lev_score = rowSums(gene_loadings[b, ]^2)),
...
get_embeddings <- function(gene_sets) {
  lapply(
    ...
    FUN = function(gene_set) {
      D <-
        sqrt((1 - cor(logcounts(combined)[gene_set,], method = "spearman")) / 2)
...
get_top_panel(get_embeddings(gene_sets[1:5]))


get_top_panel(get_embeddings(gene_sets[c(1,6:9)]))
...
get_mds_embeddings <- function(gene_sets){
  lapply(
    X = gene_sets,
    FUN = function(gene_set) {
      spearman_rho_mds(cbind(logcounts(IN[gene_set,]),
                            logcounts(LB[gene_set,]),
                            logcounts(HL[gene_set,])))
    }
  )
...
get_top_panel2(get_mnn_embeddings(gene_sets[1:5]))


get_top_panel2(get_mnn_embeddings(gene_sets[c(1,6:9)]))
```

```{r}
emb_name <- "MDS"

get_top_panel2(get_mds_embeddings(gene_sets[1:5]))


get_top_panel2(get_mds_embeddings(gene_sets[c(1,6:9)]))
